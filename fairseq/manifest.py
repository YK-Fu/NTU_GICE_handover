...
def get_parser():
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="min duration of audio"
    )
    return parser

def get_path_frame(fname, path_must_contain=None):
...
def main(args):
    ...
    with open(os.path.join(args.dest, "train.tsv"), "w") as train_f:
        ...
        for file_path, frames in Parallel(n_jobs=-1, return_as="generator")(delayed(get_path_frame)(path, args.path_must_contain) for path in tqdm(glob.glob(search_path, recursive=True))):
            if file_path is None or frames < args.min_dur:
                continue
